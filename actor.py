...
class Actor:
    ...
    def move(self, game_: 'Game', dx: int, dy: int) -> bool:
        ...
        if self.x < 0:
            self.x = 0
        elif x > game_.x_tiles:
            self.x = game_.x_tiles
        if y < 0:
            self.y = 0
        elif y > game_.y_tiles:
            self.y = game_.y_tiles

        moveables = []
        pun = game_.get_actor(x, y)
        while pun != None and pun._is_push:
            moveables.append(pun)
            x += dx
            y += dy
            pun = game_.get_actor(x, y)


        move = len(moveables) > 0 and isinstance(moveables[0], Actor)
        if pun == None:
            for act in moveables:
                act.x += dx
                act.y += dy

        if pun != None and move:
            self.x = ori_x
            self.y = ori_y
...
class Subject(Block):
    ...
    """
    def __init__(self, x, y, word_) -> None:
